          ...
          <div class="row">
              @foreach($programmations as $prog)
                <div class="col-xl-4 col-md-6">
                    ...
                    <a href="{{ url($prog->url_programmation) }}"><div class="b-actu">
                        <div class="h2">{{ $prog->titre_programmation }}</div>
                        <div class="b-actu__thumb">
                            <h2 style="background-image: url({{asset(media_url_if_not_null( $prog->media, 'Articles-ADMT-BD-FIBD', 'crops'))}}); background-repeat: no-repeat; background-size: cover; background-position: center; min-height: 200px !important; height: 16vw;">
                              <img style="opacity: 0;" src="{{asset(media_url_if_not_null( $prog->media, '0'))}}" alt="{{ media_alt_if_not_null($prog->media['libelle_media'])}}">
                            </h2>
                            <span class="btn btn-primary btn-more">EN SAVOIR PLUS</span>
                        </div>
                        @if($prog->small_description_programmation)
                        <div class="p">{!! str_limit($prog->small_description_programmation, $limit= 180, $end = '...') !!}</div>
                        @else
                        <div class="p">{!! str_limit($prog->description_programmation, $limit= 180, $end = '...') !!}</div>
                        @endif
                    </div>
                  </a>
                </div>
